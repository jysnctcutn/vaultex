...
_auth_kwargs = {}
if OAUTH_ISSUER_URL:
    # auth_server_provider/auth are constructor-only on this SDK's high-level
    # MCPServer (unlike the lowlevel server it wraps, which takes them on
    # streamable_http_app() itself) — so the OAuth wiring has to happen
    # here, at instance construction, not in core/app.py. Passing only
    # auth_server_provider (not token_verifier too) is deliberate: the SDK
    # derives a ProviderTokenVerifier from it automatically and raises if
    # both are given.
...
if OAUTH_ISSUER_URL:
    ...
    # requirement (see its docstring) — exactly what the password-gate
    # consent screen needs, since it's a public entry point by design.
...
def register_tool(func, *, name: str, description: str, write: bool = False):
    ...
    per-category tools core/tools/custom.py generates from taxonomy.json at
    import time — where the tool name isn't known until then, so it can't
