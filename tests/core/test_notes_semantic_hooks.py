"""Exercises core/notes.py's semantic-search integration points
...
embeddings-module functions core.notes imported by name -- monkeypatching
those names in core.notes' own namespace exercises the real
try/except/branch logic without needing torch inference.
"""
...
def test_auto_link_swallows_lookup_failure(semantic_enabled, monkeypatch):
    ...
    monkeypatch.setattr(notes_mod, "_embeddings_connect", _boom)
    result = _auto_link(VAULT_PATH / "new-note.md", "some body")
    assert result == "some body"


def test_infer_area_returns_default_when_deps_unavailable(monkeypatch):
    monkeypatch.setattr(notes_mod, "_SEMANTIC_DEPS_AVAILABLE", False)
...
def test_reindex_swallows_failure(semantic_enabled, monkeypatch):
    ...
    monkeypatch.setattr(notes_mod, "_index_note", _boom)
...
def test_reindex_move_deletes_old_and_indexes_new(semantic_enabled, monkeypatch):
    deleted = []
    indexed = []
    monkeypatch.setattr(notes_mod, "_delete_path", lambda conn, rel: deleted.append(rel))
    monkeypatch.setattr(
        notes_mod, "_index_note",
...
def test_reindex_move_swallows_failure(semantic_enabled, monkeypatch):
    ...
    monkeypatch.setattr(notes_mod, "_delete_path", _boom)
